...
10  TXF		TXoutput	A frame was successfully transmitted 
11  RXF		RXinput		A frame was successfully received
12  TXFIFOWERR	TXinput		TX (memory) FIFO write error
13  RXFIFOWERR	RXinput		RX (memory) FIFO write error 
14  RXPHYERR	RXinput		RX phy error, caused by PHY asserting RX_ER
15  RXOFERR	RXinput		RX small async fifo overflow
16  RXCRCERR	RXinput		RX valid packet received with bad checksum


We also have the:
08 PHY address register (write)
09 PHY data register
0A PHY status (0 means previous address write/read is complete, and data has been updated to reflect this)
